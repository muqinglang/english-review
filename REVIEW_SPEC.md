...
- 网站在用户揭晓答案后接受自评：`答错 (incorrect)`、`模糊 (partial)`、`答对 (correct)`。旧题可以在任意一天重新自评（昨天答对不代表今天记得），每次自评都按学习项当前状态重跑排期。幂等边界是“同一学习项 + 同一上海日”：同一天多次自评以最后一次为准（回退到当天首次自评前的基线再套用最新结果，`attempts`/`correct` 不因当天重评而叠加），跨天的自评则从当前状态继续推进。相同 `requestId` 的重复提交仍然幂等。
- 答错：阶段重置为 0，次日复习；模糊：阶段降低一级（最低 0），3 天后复习；答对：阶段依次推进到 1、2、3，对应 7、30、90 天后复习。到达阶段 3 后标记为 `mastered`，但到期时仍可复习。
- 每次计入的自评都增加 `attempts`，只有答对增加 `correct`；揭晓答案本身不算作答，也不改变排期。同一天的重复自评在回退基线后重算，不会重复累加 `attempts`。
