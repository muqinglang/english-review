...
const rateOptions: { result: AttemptResult; label: string; base: string; ring: string }[] = [
  { result: "incorrect", label: "Forgot · try tomorrow", base: "border-[#e2b7ad] bg-[#fff8f6] text-danger hover:bg-[#fcebe7]", ring: "ring-[#c87563]" },
  { result: "partial", label: "Unsure · review soon", base: "border-[#dec991] bg-[#fffaf0] text-[#80631c] hover:bg-[#fbf1d6]", ring: "ring-[#c6a54e]" },
  { result: "correct", label: "Got it · longer interval", base: "border-[#a9cbb7] bg-mint text-primary-strong hover:bg-[#e2f3e8]", ring: "ring-[#5a9b78]" },
];
...
function ExamplePlayButton({ id, text, playingId, onPlay }: { id: string; text: string; playingId: string; onPlay: (id: string, text: string) => void }) {
  ...
  return <button
    ...
    className={`grid size-7 shrink-0 place-items-center rounded-control border transition ${active ? "border-primary bg-mint text-primary-strong" : "border-line bg-surface text-primary hover:bg-mint"}`}
  >
...
function InlineText({ text }: { text: string }) {
  ...
  return <>{parts.map((part, index) => {
    if (part.startsWith("`") && part.endsWith("`")) {
      return <code key={index} className="rounded bg-mint px-1.5 py-0.5 font-mono text-[0.92em] text-primary-strong">{part.slice(1, -1)}</code>;
    }
    if (part.startsWith("**") && part.endsWith("**")) {
...
function MarkdownReview({ markdown }: { markdown: string }) {
  return <div className="space-y-3 text-[15px] leading-7 text-muted">{markdown.split(/\r?\n/).map((raw, index) => {
    const line = raw.trim();
    if (!line) return <div key={index} className="h-2" />;
    if (line.startsWith("# ")) return <h2 key={index} className="text-2xl font-black text-ink">{line.slice(2)}</h2>;
    if (line.startsWith("## ")) return <h3 key={index} className="mt-8 border-t border-line pt-7 text-xl font-black text-ink">{line.slice(3)}</h3>;
    if (line.startsWith("### ")) return <h4 key={index} className="mt-5 text-base font-extrabold text-primary">{line.slice(4)}</h4>;
    const numbered = line.match(/^(\d+)\.\s+(.*)$/);
    if (numbered) {
      return <div key={index} className="flex gap-3"><span className="mt-0.5 flex h-6 min-w-6 items-center justify-center rounded-full bg-mint text-xs font-black text-primary">{numbered[1]}</span><p><InlineText text={numbered[2]} /></p></div>;
    }
    if (line.startsWith("- ")) return <div key={index} className="flex gap-3 pl-2"><span className="text-primary">•</span><p><InlineText text={line.slice(2)} /></p></div>;
...
function LatestConversationReview({ items, reviewDate, isLatest, onPlay, playingId }: { items: YesterdayConversationItemData[]; reviewDate: string | null; isLatest: boolean; onPlay: (id: string, text: string) => void; playingId: string }) {
  ...
  if (!items.length) {
    return <section className="mt-5 rounded-card border border-dashed border-line bg-[#f8fbf8] p-5 sm:p-6" aria-labelledby="latest-conversation-title">
      <p id="latest-conversation-title" className="text-lg font-black text-primary-strong">Latest conversation review</p>
      <p className="mt-2 text-sm leading-6 text-muted">No ChatGPT conversation has synced yet. We won&apos;t fall back to an old daily review pack.</p>
    </section>;
  }
  ...
  return <section className="mt-5 rounded-card border border-line bg-[#f5faf5] p-5 sm:p-6" aria-labelledby="latest-conversation-title">
    <div className="flex flex-wrap items-baseline justify-between gap-2">
      <div>
        <p id="latest-conversation-title" className="text-lg font-black text-primary-strong">{isLatest ? "Latest conversation review" : "Conversation review"}</p>
        <p className="mt-1 text-sm leading-6 text-muted">{items.length} items synced from {reviewDate}. Answer in your head first, then rate yourself — results enter the spaced-repetition history.</p>
      </div>
      <span className="rounded-full bg-surface px-3 py-1 text-xs font-extrabold text-primary">Conversation sync</span>
    </div>
    <div className="mt-5 grid gap-3">
      {items.map((item, index) => {
        const richAnswer = richAnswers.get(item.id) ?? parseRichAnswer(item);
        const isRevealed = revealed[item.id] ?? false;
        return <article key={item.id} className="rounded-card border border-line bg-surface p-4">
          <p className="text-xs font-extrabold tracking-[0.1em] text-primary">Item {index + 1}</p>
          <h3 className="mt-2 font-black leading-7 text-ink">{item.cue}</h3>
          {!isRevealed ? <Button type="button" variant="secondary" size="sm" onClick={() => setRevealed((current) => ({ ...current, [item.id]: true }))} className="mt-3">Show answer</Button> : <div className="mt-4 overflow-hidden rounded-card border border-line">
            <section className="p-4">
              <p className="text-xs font-extrabold tracking-[0.12em] text-faint">Core meaning</p>
              <p className="mt-2 text-base font-bold leading-7 text-ink">{richAnswer.meaning}</p>
            </section>
            {richAnswer.explanation && <section className="border-t border-line bg-[#fbfcfa] p-4">
              <p className="text-xs font-extrabold tracking-[0.12em] text-faint">How to think of it</p>
              <p className="mt-2 text-sm leading-7 text-muted">{richAnswer.explanation}</p>
            </section>}
            {richAnswer.examples.length > 0 && <section className="border-t border-line p-4">
              <p className="text-xs font-extrabold tracking-[0.12em] text-faint">In real life</p>
              <p className="mt-1 text-xs leading-5 text-faint">Listen first; if you can&apos;t catch it, tap the eye to reveal the English and translation.</p>
              <div className="mt-3 grid gap-3 lg:grid-cols-2">
                {richAnswer.examples.map((example, exampleIndex) => {
                  ...
                  return <article key={`${example.english}-${exampleIndex}`} className="rounded-card bg-[#f3f7f2] p-4">
                    <div className="flex items-start justify-between gap-2">
                      <p className="text-xs font-extrabold text-primary">{example.scenario ?? `Scene ${exampleIndex + 1}`}</p>
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          ...
                          className="grid size-7 shrink-0 place-items-center rounded-control border border-line bg-surface text-primary transition hover:bg-mint"
                        >
                    ...
                    {exampleShown ? <>
                      <p className="mt-2 text-sm font-semibold leading-6 text-ink">{example.english}</p>
                      {example.chinese && <p className="mt-2 text-sm leading-6 text-muted">{example.chinese}</p>}
                    </> : <p className="mt-2 text-sm leading-6 text-faint">Text hidden · listen first, tap the eye at the top-right to reveal.</p>}
                  </article>;
                })}
              </div>
            </section>}
            {richAnswer.usageTip && <section className="border-t border-line bg-[#fffaf0] p-4">
            ...
            <div className="border-t border-line bg-mint p-4">
              {results[item.id] ? <p className="font-extrabold text-primary-strong">Rated: {resultLabels[results[item.id]]} — added to the review schedule.</p> : item.graded ? <p className="font-extrabold text-primary-strong">Rated — added to the review schedule. Continue in “Review History”.</p> : <div className="flex flex-wrap gap-2">
                {(["incorrect", "partial", "correct"] as AttemptResult[]).map((result) => <button key={result} type="button" disabled={submitting[item.id]} onClick={() => submit(item, result)} className={`rounded-control border px-3 py-2 text-xs font-extrabold transition disabled:opacity-50 ${result === "incorrect" ? "border-[#e2b7ad] bg-[#fff8f6] text-danger hover:bg-[#fcebe7]" : result === "partial" ? "border-[#dec991] bg-[#fffaf0] text-[#80631c] hover:bg-[#fbf1d6]" : "border-[#a9cbb7] bg-mint text-primary-strong hover:bg-[#e2f3e8]"}`}>{submitting[item.id] ? "Saving…" : resultLabels[result]}</button>)}
              </div>}
              {errors[item.id] && <p className="mt-2 font-semibold text-danger">{errors[item.id]}</p>}
            </div>
          </div>}
        </article>;
      })}
...
function ReviewCards({
  ...
  return <div className="space-y-4">
    <div className="rounded-card bg-mint p-4 text-sm leading-6 text-primary-strong">
    ...
    {cards.map((card, index) => {
      ...
      return <article key={card.reviewItemId} aria-labelledby={titleId} aria-busy={state.submitting || undefined} className="rounded-card border border-line bg-[#fcfdfb] p-5 sm:p-6">
        <p className="text-xs font-extrabold tracking-[0.12em] text-primary">Item {index + 1} of {cards.length}</p>
        <h3 id={titleId} className="mt-3 text-lg font-black leading-8 text-ink">{card.cue}</h3>
          ...
          <div className="mt-5 space-y-4">
            <div className="overflow-hidden rounded-card border border-line bg-surface">
              <section className="p-4 sm:p-5" aria-labelledby={`${titleId}-meaning`}>
                <p id={`${titleId}-meaning`} className="text-xs font-extrabold tracking-[0.12em] text-faint">Core meaning</p>
                <p className="mt-2 whitespace-pre-wrap text-base font-bold leading-7 text-ink">{richAnswer.meaning}</p>
              </section>

              {richAnswer.explanation && <section className="border-t border-line bg-[#fbfcfa] p-4 sm:p-5" aria-labelledby={`${titleId}-explanation`}>
                <p id={`${titleId}-explanation`} className="text-xs font-extrabold tracking-[0.12em] text-faint">How to think of it</p>
                <p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-muted">{richAnswer.explanation}</p>
              </section>}

              {richAnswer.examples.length > 0 && <section className="border-t border-line p-4 sm:p-5" aria-labelledby={`${titleId}-examples`}>
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  <p id={`${titleId}-examples`} className="text-xs font-extrabold tracking-[0.12em] text-faint">In real life</p>
                  <p className="text-xs font-semibold text-faint">{richAnswer.examples.length} total</p>
                </div>
                <p className="mt-1 text-xs leading-5 text-faint">Listen first; if you can&apos;t catch it, tap the eye to reveal the English and translation.</p>
                <div className="mt-3 grid gap-3 lg:grid-cols-2">
                  {richAnswer.examples.map((example, exampleIndex) => {
                    ...
                    return <article key={`${example.english}-${exampleIndex}`} className="rounded-card bg-[#f3f7f2] p-4">
                      ...
                      {exampleShown ? <>
                        <p className="mt-2 whitespace-pre-wrap text-sm font-semibold leading-6 text-ink">{example.english}</p>
                        {example.chinese && <p className="mt-1.5 whitespace-pre-wrap text-sm leading-6 text-muted">{example.chinese}</p>}
                      </> : <p className="mt-2 text-sm leading-6 text-faint">Text hidden · listen first, tap the eye at the top-right to reveal.</p>}
                    </article>;
                  })}
                </div>
              ...
              {richAnswer.usageTip && <section className="border-t border-line bg-[#fffaf0] p-4 sm:p-5" aria-labelledby={`${titleId}-usage-tip`}>
            ...
            {lastRecorded && (
              <div role="status" className="rounded-card bg-mint p-4 text-sm leading-6 text-primary">
            ...
            <fieldset disabled={state.submitting}>
              <legend className="text-sm font-extrabold text-muted">{lastRecorded ? "Rate again today (yesterday's “Got it” doesn't mean you remember today — update anytime)" : "How well did you recall it?"}</legend>
              <div className="mt-3 flex flex-wrap gap-2">
                {rateOptions.map((option) => {
                  const isCurrent = lastRecorded === option.result;
                  return <button key={option.result} type="button" aria-pressed={isCurrent} onClick={() => onSubmit(card, option.result)} className={`rounded-control border px-4 py-2.5 text-sm font-bold transition disabled:cursor-wait disabled:opacity-55 ${option.base} ${isCurrent ? `ring-2 ring-offset-1 ${option.ring}` : ""}`}>{isCurrent ? "✓ " : ""}{option.label}{isCurrent ? " · saved" : ""}</button>;
                })}
              </div>
              {state.submitting && <p aria-live="polite" className="mt-3 text-sm font-bold text-primary">Saving your rating…</p>}
            </fieldset>
  ...
  return <>
    ...
    <section className="min-w-0 rounded-card border border-line bg-surface p-5 sm:p-8">
      <div className="flex gap-1 rounded-card bg-[#f3f6f2] p-1.5" role="tablist" aria-label="Review content">
        <button type="button" role="tab" aria-selected={activeTab === "conversation"} onClick={() => { cancelPlayback(); setActiveTab("conversation"); }} className={`flex flex-1 items-center justify-center gap-1.5 rounded-control px-3 py-2.5 text-sm font-extrabold transition ${activeTab === "conversation" ? "bg-surface text-ink shadow-sm" : "text-faint hover:text-muted"}`}><ChatCircleDots size={17} />Conversation</button>
        <button type="button" role="tab" aria-selected={activeTab === "history"} onClick={() => { cancelPlayback(); setActiveTab("history"); }} className={`flex flex-1 items-center justify-center gap-1.5 rounded-control px-3 py-2.5 text-sm font-extrabold transition ${activeTab === "history" ? "bg-surface text-ink shadow-sm" : "text-faint hover:text-muted"}`}><Stack size={17} />Review History</button>
        <button type="button" role="tab" aria-selected={activeTab === "audio"} onClick={() => setActiveTab("audio")} className={`flex flex-1 items-center justify-center gap-1.5 rounded-control px-3 py-2.5 text-sm font-extrabold transition ${activeTab === "audio" ? "bg-surface text-ink shadow-sm" : "text-faint hover:text-muted"}`}><Headphones size={17} />Listening</button>
      </div>

      {ttsProvider === "elevenlabs" && elevenLabsVoices.length > 0 && <label className="mt-4 flex flex-wrap items-center gap-2 text-sm font-bold text-muted">
        <SpeakerHigh size={16} className="text-primary" />Voice
        <select value={selectedVoiceId} onChange={(event) => chooseVoice(event.target.value)} className="rounded-control border border-line bg-surface px-3 py-2 text-sm">
          {elevenLabsVoices.map((voice) => <option key={voice.voiceId} value={voice.voiceId}>{voice.name}</option>)}
        </select>
        <span className="text-xs font-medium text-faint">ElevenLabs · this voice plays every clip below</span>
      </label>}

      {activeTab === "conversation" ? <>
        <ConversationImport />
        {library.conversationSessions.length > 1 && <label className="mt-4 flex flex-wrap items-center gap-2 text-sm font-bold text-muted">
          <ChatCircleDots size={16} className="text-primary" />Pick a date
          <select value={conversationSession?.id ?? ""} onChange={(event) => chooseConversationSession(event.target.value)} className="rounded-control border border-line bg-surface px-3 py-2 text-sm">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-line pb-6">
          <div>
            <p className="text-xs font-extrabold tracking-[0.14em] text-primary">{review.date} · {review.level} · {review.duration} min</p>
            <h2 className="mt-2 text-2xl font-black sm:text-3xl">{review.title}</h2>
          </div>
          {library.reviews.length > 1 && <label className="text-sm font-bold text-muted">
            <span className="sr-only">Pick a review date</span>
            <select value={review.id} onChange={(event) => chooseReview(event.target.value)} className="rounded-control border border-line bg-surface px-3 py-2 text-sm">
        ...
        <div className="mt-8">{activeTab === "history" ? (
          ...
          <div className="space-y-4">
            ...
            {audioNotice && <Alert tone="warning">{audioNotice}</Alert>}
            ...
            {(() => {
              ...
              return <article className="rounded-card border border-line bg-mint p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-xs font-extrabold tracking-[0.1em] text-primary">STORY · Today&apos;s review words</p>
                    <p className="mt-1 text-sm leading-6 text-muted">A short story woven from the words you have due for review. Generate it, listen, and write down what you hear.</p>
                  </div>
                  <Button
                    type="button"
                    variant="primary"
                ...
                {storyError && <Alert tone="error" className="mt-3">{storyError}</Alert>}

                {story && <>
                  <div className="mt-4 flex flex-wrap gap-2">
                    <button type="button" aria-label="Play the story at normal speed" title="Normal speed" onClick={() => playText(`${review.id}:story:normal`, story, 1)} className="grid size-10 place-items-center rounded-control bg-ink text-white transition hover:bg-[#2e403c] active:translate-y-px"><Play size={17} weight="fill" /></button>
                    <button type="button" aria-label="Play the story at slow speed" title="Slow speed" onClick={() => playText(`${review.id}:story:slow`, story, 0.75)} className="flex h-10 items-center gap-1.5 rounded-control border border-line px-3 text-xs font-bold text-primary transition hover:bg-mint active:translate-y-px"><SpeakerHigh size={16} />0.75×</button>
                    <button
                      type="button"
                      ...
                      className="inline-flex items-center gap-1.5 rounded-control border border-line bg-surface px-3 py-2 text-xs font-extrabold text-primary transition hover:bg-mint"
                    >
                  ...
                  <label htmlFor={storyInputId} className="mt-5 block text-sm font-extrabold text-muted">Write what you hear</label>
                  <Textarea
                    id={storyInputId}
                    value={storyDraft}
                    ...
                    className="mt-2 rounded-card bg-[#fbfcfa]"
                  />

                  <div id={storyTranscriptId} className={`mt-4 rounded-card border px-4 py-3 ${storyTranscriptVisible ? "border-line bg-[#f1f8f3]" : "border-dashed border-line bg-[#fafbf9]"}`}>
                    {storyTranscriptVisible ? <>
                      <p className="text-xs font-extrabold tracking-[0.1em] text-primary">Story text</p>
                      <p className="mt-2 text-base font-bold leading-7 text-ink">{story}</p>
                    </> : <p className="text-sm leading-6 text-muted">Story text hidden. When you&apos;re done, tap “Show text” to check.</p>}
                  </div>
                </>}
              </article>;
            })()}
            {review.audioCards.map((card, index) => {
              ...
              return <article key={stateKey} className="rounded-card border border-line p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <p className="text-xs font-extrabold text-primary">{String(index + 1).padStart(2, "0")} · {card.prompt}</p>
                  <button
                    type="button"
                    ...
                    className="inline-flex items-center gap-1.5 rounded-control border border-line bg-surface px-3 py-2 text-xs font-extrabold text-primary transition hover:bg-mint focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
                  >
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <button type="button" aria-label="Play at normal speed" title="Normal speed" onClick={() => playAudio(normalPlaybackId, review.id, card.id, "normal", card.normal, 1)} className="grid size-10 place-items-center rounded-control bg-ink text-white transition hover:bg-[#2e403c] active:translate-y-px"><Play size={17} weight="fill" /></button>
                ...
                <label htmlFor={inputId} className="mt-5 block text-sm font-extrabold text-muted">Write what you hear</label>
                <Textarea
                  id={inputId}
                  value={draft}
                ...
                <div id={transcriptId} className={`mt-4 rounded-card border px-4 py-3 ${transcriptVisible ? "border-line bg-[#f1f8f3]" : "border-dashed border-line bg-[#fafbf9]"}`}>
                  {transcriptVisible ? <>
                    <p className="text-xs font-extrabold tracking-[0.1em] text-primary">English text</p>
                    <p className="mt-2 text-base font-bold leading-7 text-ink">{card.normal}</p>
                  </> : <p className="text-sm leading-6 text-muted">English text hidden. When you&apos;re done, tap the eye at the top-right to check.</p>}
                </div>
              </article>;
            })}
          </div>
        )}</div>
      </> : <div className="py-16 text-center"><p className="text-lg font-black">No daily review for this category yet</p><p className="mt-2 text-sm text-faint">It&apos;ll show up here once the Worker pushes the next review pack.</p></div>}
    </section>
    </div>
  </>;
}
